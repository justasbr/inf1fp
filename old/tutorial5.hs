...
isBloodOrange (Orange(variety, segments)) = variety `elem` ["Tarocco", "Moro", "Sanguinello"]
isBloodOrange _ = False

-- 2.
--bloodOrangeSegments :: [Fruit] -> Int
--bloodOrangeSegments [] = 0
--bloodOrangeSegments ((Orange(variety, segments)):fruits) = (if isBloodOrange (Orange(variety,segments)) then segments else 0) + bloodOrangeSegments fruits
--bloodOrangeSegments (_:fruits) = bloodOrangeSegments fruits
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments fruits = sum [segments fruit | fruit <- fruits, isBloodOrange fruit]
    where segments (Orange(a,b)) = b

-- 3.
worms :: [Fruit] -> Int
worms fruits = length [1 | fruit <- fruits, isAppleWithWorm fruit]
	where 
	isAppleWithWorm (Apple(variety, hasWorm)) = hasWorm
...
equivalent p1 p2 = and [(eval env p1 == eval env p2) | env <- envs (nub (names p1 ++ names p2))]
